...
class ControllerExtensionPaymentSwipe extends Controller {
    public function index() {
        ...
        if(false){
            ...
            if ($this->config->get('payment_swipe_live_demo') == 1) {
                $data['action'] = $this->config->get('payment_swipe_live_url');
            } else {
                $data['action'] = $this->config->get('payment_swipe_demo_url');
            }

            if ($this->config->get('payment_swipe_card_select') == 1) {
                $card_types = array(
                ...
                $accounts = $this->config->get('payment_swipe_account');

                foreach ($accounts as $card => $account) {
                    if (isset($account['enabled']) && $account['enabled'] == 1) {
                        $data['cards'][] = array(
                            'type' => $card_types[$card],
                            'account' => (isset($account['default']) && $account['default'] == 1 ? $this->config->get('payment_swipe_merchant_id') : $account['merchant_id']),
                        );
                    }
                }
            ...
            if ($this->config->get('payment_swipe_auto_settle') == 0) {
                $data['settle'] = 0;
            } elseif ($this->config->get('payment_swipe_auto_settle') == 1) {
                $data['settle'] = 1;
            } elseif ($this->config->get('payment_swipe_auto_settle') == 2) {
                $data['settle'] = 'MULTI';
            }
            $data['tss'] = (int)$this->config->get('payment_swipe_tss_check');
            $data['merchant_id'] = $this->config->get('payment_swipe_merchant_id');
        ...
        $data['redirect'] = $payment_issue['redirect'];
        $data['response_url'] = HTTPS_SERVER . 'index.php?route=extension/payment/swipe/notify';

        return $this->load->view('extension/payment/swipe', $data);
    }

    // Process the payment
    public function process_payment( $order_id,$order_info = array() ) {
        ...
        try {
            if ( $payment_url = $this->get_payment_link( $order_id ) ) {
                return array(
                    'result'   => 'success',
    ...
    {
        $this->se( $this->config->get( 'api_key' ) );
    ...
    public function notify() {
        ...
        $myfile = fopen("newfile".rand(222,333).".json", "w") or die("Unable to open file!");
        $txt = json_decode($this->request->post);
        fwrite($myfile, $txt);
        ...
        $myfile = fopen("newfile".rand(222,333).".json", "w") or die("Unable to open file!");
        $txt = json_decode($this->request->get);
        fwrite($myfile, $txt);
        fclose($myfile);



        $myfile = fopen("mnewfile".rand(222,333).".json", "w") or die("Unable to open file!");
        $txt = json_decode($this->request->post);
        fwrite($myfile, $txt);
        ...
        $this->load->model('extension/payment/swipe');

        $this->model_extension_payment_swipe->logger(print_r($this->request->post, 1));

        $this->load->language('extension/payment/swipe');

        $hash = sha1($this->request->post['TIMESTAMP'] . '.' . $this->config->get('payment_swipe_merchant_id') . '.' . $this->request->post['ORDER_ID'] . '.' . $this->request->post['RESULT'] . '.' . $this->request->post['MESSAGE'] . '.' . $this->request->post['PASREF'] . '.' . $this->request->post['AUTHCODE']);
        $tmp = $hash . '.' . $this->config->get('payment_swipe_secret');
        $hash = sha1($tmp);
        ...
        if ($hash != $this->request->post['SHA1HASH']) {
            ...
            $auto_settle = (int)$this->config->get('payment_swipe_auto_settle');
            $tss = (int)$this->config->get('payment_swipe_tss_check');
            ...
            if ($this->request->post['RESULT'] == "00") {
                $swipe_order_id = $this->model_extension_payment_swipe->addOrder($order_info, $this->request->post['PASREF'], $this->request->post['AUTHCODE'], $this->request->post['ACCOUNT'], $this->request->post['ORDER_ID']);

                if ($auto_settle == 1) {
                    $this->model_extension_payment_swipe->addTransaction($swipe_order_id, 'payment', $order_info);
                    $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('swipe_order_status_success_settled_id'), $message, false);
                } else {
                    $this->model_extension_payment_swipe->addTransaction($swipe_order_id, 'auth', 0.00);
                    $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_swipe_order_status_success_unsettled_id'), $message, false);
                }
                ...
                // Decline
                $this->model_extension_payment_swipe->addHistory($order_id, $this->config->get('payment_swipe_order_status_decline_id'), $message);
                $data['text_response'] = $this->language->get('text_decline');
                $data['text_link'] = sprintf($this->language->get('text_link'), $this->url->link('checkout/checkout', '', true));
            } elseif ($this->request->post['RESULT'] == "102") {
                // Referal B
                $this->model_extension_payment_swipe->addHistory($order_id, $this->config->get('payment_swipe_order_status_decline_pending_id'), $message);
                $data['text_response'] = $this->language->get('text_decline');
                $data['text_link'] = sprintf($this->language->get('text_link'), $this->url->link('checkout/checkout', '', true));
            } elseif ($this->request->post['RESULT'] == "103") {
                // Referal A
                $this->model_extension_payment_swipe->addHistory($order_id, $this->config->get('payment_swipe_order_status_decline_stolen_id'), $message);
                $data['text_response'] = $this->language->get('text_decline');
                $data['text_link'] = sprintf($this->language->get('text_link'), $this->url->link('checkout/checkout', '', true));
            } elseif ($this->request->post['RESULT'] == "200") {
                // Error Connecting to Bank
                $this->model_extension_payment_swipe->addHistory($order_id, $this->config->get('payment_swipe_order_status_decline_bank_id'), $message);
        ...
        $this->response->setOutput($this->load->view('extension/payment/swipe_response', $data));
    }
}